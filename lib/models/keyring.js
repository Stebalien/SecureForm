...
var store = require('lib/backbone-simplestore');
...
var Keyring = Backbone.RelationalModel.extend({
  ...
  initialize: function() {
    ...
    this.set({
      ...
      publicKeys: _.map(pgpKeyring.publicKeys, function(key) {
        return {pgpKey: key};
      })
    });
  ...
  importPrivateKey: function(key, password) {
    ...
    if (pgpKeyring.importPrivateKey(key, password)) {
      this.get('privateKeys').add(_.compact(_.map(pgpKeyring.privateKeys.slice(index), function(key) {
        try {
