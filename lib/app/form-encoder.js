...
function encodeWithPadding(myWin, mySubmit, callback) {
  ...
  var getData = function(encoded) {
    ...
    var desc = getAllDescendants(myDoc, new Array());
    var cloneDesc = getAllDescendants(cloned, new Array());
    for (var i=0; i<desc.length; i++) {
      if (cloneDesc[i].nodeName === "TEXTAREA") {
        cloneDesc[i].value = desc[i].value;
      }
      else if (cloneDesc[i].nodeName === "INPUT" && cloneDesc[i].type === "file") {
        cloneDesc[i].data = desc[i].data;
      }
    }
...
function encodeInside(myWin, myOuterForm, mySubmit) {
  ...
  while (elem != null) {
    var processed = process(myWin, elem, false, boundary);
    if (processed !== null) {
      encoded += ("--" + boundary + "\r\n");
      encoded += (processed + "\r\n");
    }
...
function processForm(myWin, myForm) {
  ...
  for (var i=0; i<desc.length; i++) {
    var processed = process(myWin, desc[i], true, boundary);
    if (processed !== null) {
      encoded += ("--" + boundary + "\r\n");
      encoded += (processed + "\r\n");
    }
...
function process(myWin, elem, isForm, boundary) {
  ...
  if (elem.disabled || (!elem.name && !(elem.parentNode.nodeName === "BODY"))) {
    return null;
  }
...
function processFile(myWin, myFile, isForm, outerBoundary) {
  var part = "";
  var fileReader = new myWin.FileReader();
