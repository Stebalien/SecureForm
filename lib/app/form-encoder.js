...
function encodeWithPadding(myDoc, mySubmit, callback) {
  var getData = function(encoded) {
    ...
    encodeSecuredDocument(maxDoc, mySubmit, getMaxData);
  }
  encodeSecuredDocument(myDoc, mySubmit, getData);
}

exports.encodeWithPadding = encodeWithPadding;
...
function encodeSecuredDocument(myDoc, mySubmit, callback) {   
  ...
  function readFile(index) {
    if (index >= files.length) {
      var encoded = encodeInside(myOuterForm, mySubmit);
      callback(encoded);
      return;
    }
...
function encodeInside(myOuterForm, mySubmit) {
  var encoded = "";
  ...
  while (elem != null) {
    processed = process(elem, false, boundary);
    if (processed !== null) {
      encoded += ("--" + boundary + "\r\n");
      encoded += (processed + "\r\n");
    }
...
function processForm(myForm) {
  ...
  for (var i=0; i<desc.length; i++) {
    processed = process(desc[i], true, boundary);
    if (processed !== null) {
      encoded += ("--" + boundary + "\r\n");
      encoded += (processed + "\r\n");
    }
...
function process(elem, isForm, boundary) {
  // disabled form elements shouldn't be submitted
  // neither should form elements that don't have a name
  if (elem.disabled || elem.name === undefined) {
    return null;
  }

  if (elem.nodeName === "INPUT") {
    if (elem.type === "text") {
      return processText(elem, isForm);
    }
    else if (elem.type === "password") {
      return processPassword(elem, isForm);
    }
    else if (elem.type === "radio") {
      return processRadio(elem, isForm);
    }
    else if (elem.type === "checkbox") {
      return processCheckbox(elem, isForm);
    }
    else if (elem.type === "file") {
      return processFile(elem, isForm, boundary);
    }
    else if (elem.type === "submit") {
      return processSubmit(elem, isForm);
    }
  }
  else if (elem.nodeName === "SELECT") {
    return processSelect(elem, isForm, boundary);
  }
  else if (elem.nodeName === "TEXTAREA") {
    return processTextarea(elem, isForm);
  }
  else if (elem.nodeName === "FORM") {
    return processForm(elem);
  }
...
function processTextElem(elem, isForm) {
  part = "";
  part += (PLAINTEXT_TYPE + "; ");
  part += (UTF8_CHARSET + "\r\n")
  if (isForm) {
...
function processTextarea(myTextarea, isForm) {
  return processTextElem(myTextarea, isForm);
}

function processSelect(mySelect, isForm, boundary) {
  part = "";
...
function processText(myText, isForm) {
  return processTextElem(myText, isForm);
}

function processPassword(myPassword, isForm) {
  return processTextElem(myPassword, isForm);
}

function processRadio(myRadio, isForm) {
  // radio buttons that aren't selected aren't submitted
  if (!myRadio.checked) {
    return null;
  }
  return processTextElem(myRadio, isForm);
}

function processCheckbox(myCheckbox, isForm) {
  // checkboxes that aren't selected also aren't submitted
  if (!myCheckbox.checked) {
    return null;
  }
  return processTextElem(myCheckbox, isForm);
}

function processSubmit(mySubmit, isForm) {
...
function processFile(myFile, isForm, outerBoundary) {
  part = "";
  ...
  if (myFile.files.length === 0) {
    ...
    if (myFile.multiple) {
      ...
      for (var i=0; i<myFile.files.length; i++) {
        ...
        part += processOneFile(selectedFile, isForm, null);
      }
      part += ("--" + boundary + "--\r\n");
    }
    else {
      console.log(myFile);
      part += processOneFile(myFile.files[0], isForm, myFile.name);
    }
  }
  else {
    for (var i=0; i<myFile.files.length; i++) {
...
function processOneFile(selectedFile, isForm, elemName) {
  part = "";
