...
var PublicKey = Key.extend({
  ...
  fromPgpKey: function(pgpKey) {
    ...
    var key = PublicKey.findOrCreate({
      id: primaryKey.id,
    ...
    key.get('primaryKey').set('masterKey', key);
    key.get('subKeys').each(function(sk) {
      sk.set('masterKey', key);
    });
...
var PrivateSubKey = Backbone.RelationalModel.extend({
  ...
  fromPgpKey: function(pgpKey) {
    var id = convertId(pgpKey.publicKey.getKeyId());
...
var PrivateKey = Key.extend({
  ...
  fromPgpKey: function(pgpKey) {
    ...
    var key = PrivateKey.findOrCreate({
      id: primaryKey.id,
      uids: _.map(pgpKey.userIds, function(uid) {
        return Uid.fromPgpUid(uid);
      }),
