...
exports.PublicKeyringView = KeyringView.extend({
  ...
  events: {
    'keyup #publicKeySearch': 'onSearchChange'
  },
  render: function() {
    this.$el.append("<input type='text' id='publicKeySearch'>");
    this.$el.append(this.listView.render().$el);
    return this;
  },
  onSearchChange: _.debounce(function() {
    var terms = _.str.words(this.this.$("#publicKeySearch").val());
    this.collection.filterBy(function(model) {
      model.get('uids').any(function(uid) {
        return _.every(terms, function(word) {
          return uid.get('uid').contains(word);
        });
      });
    });
  })
